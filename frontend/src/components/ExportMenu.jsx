...
function ExportMenu({ result, idea }) {
  ...
  const handleDownloadPDF = async () => {
    ...
    setExportProgress(0);
    const loadingToast = toast.loading('Compiling comprehensive PDF...');
    ...
    try {
      ...
      const addFooter = () => {
        ...
        doc.text(
          `Page ${currentPage} | ARCHITECH.AI COMPILED SPECIFICATION`,
          pageWidth / 2,
          pageHeight - 10,
          { align: 'center' }
        );
      ...
      // Cover Page - Neo-Brutalist
      doc.setFillColor(253, 246, 227); // Cream
      ...
      doc.setTextColor(0, 0, 0);
      doc.text('SYSTEM ARCHITECTURE SPEC', pageWidth / 2, 70, { align: 'center' });
      
      doc.setFontSize(14);
      doc.setFont('helvetica', 'normal');
      const ideaLines = doc.splitTextToSize(idea, contentWidth - 40);
      doc.text(ideaLines, pageWidth / 2, 95, { align: 'center', maxWidth: contentWidth - 40 });
      
      doc.setFontSize(11);
      doc.text(`COMPILED: ${new Date().toLocaleString()}`, pageWidth / 2, pageHeight - 60, { align: 'center' });
      
      doc.setFontSize(10);
      doc.text('GENERATED VIA ARCHITECH.AI (V2.0)', pageWidth / 2, pageHeight - 40, { align: 'center' });
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(0, 0, 0);
      doc.text('1. Features [MoSCoW]', margin, 30);
      ...
      const techStackData = [
        ...
        ['Architecture Pattern', result.architecture?.type || 'N/A']
      ];
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(0, 0, 0);
      doc.text('3. Database Schema', margin, 30);
      ...
      if (result.database?.length) {
        ...
        result.database.forEach((table) => {
          ...
          doc.setFontSize(13);
          doc.setFont('helvetica', 'bold');
          doc.text(`Table: ${table.table}`, margin, tableY);
          tableY += 8;
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(0, 0, 0);
      doc.text('4. REST API Endpoints', margin, 30);
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(0, 0, 0);
      doc.text('5. Development Roadmap', margin, 30);
      ...
      if (result.roadmap?.length) {
        ...
        result.roadmap.forEach((phase, idx) => {
          ...
          phase.tasks?.forEach(task => {
            ...
            if (roadmapY + taskLines.length * 4 > pageHeight - 30) {
              addNewPage();
              roadmapY = 30;
            }
            doc.text(taskLines, margin + 5, roadmapY);
            roadmapY += taskLines.length * 4 + 2;
          });
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(0, 0, 0);
      doc.text('6. Project Estimation & Budget', margin, 30);
